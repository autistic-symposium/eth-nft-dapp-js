...
import MINT_CONTRACT from '../../artifacts/FilmmakerDAO.json';
...
const DirectMint = ({ developerId }: DirectMintProps) => {
  ...
  useEffect(() => {
    const web3Modal = new Web3Modal({
      network:  FILMMAKER_DAO_CONTRACT_NETWORK,
      cacheProvider: false,
      providerOptions,
    });
  ...
  const createTokenHandler: MouseEventHandler<HTMLButtonElement> = async (
    ...
    try {
      ...
      const tx = await mint_contract.mintCard(tokenID, overrides);
